...
const { LocalProjects } = jest.requireActual('../src/native/LocalProjects') as
  typeof import('../src/native/LocalProjects');
...
test.each([
  ...
  ['mismatched workspace path', { workspace_path: `projects/${PROJECT_ID}/other` }],
  ['oversized name', { name: 'x'.repeat(121) }],
  ['unsafe timestamp', { updated_at: '2026-02-30T00:00:00.000Z' }],
  ['credential URL', { origin_url: 'https://token@example.com/repository.git' }],
  ['query URL', { origin_url: 'https://example.com/repository.git?token=secret' }],
] as const)('rejects %s without returning native values', async (_label, change) => {
