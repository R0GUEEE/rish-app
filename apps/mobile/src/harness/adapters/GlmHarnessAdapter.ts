...
import { GLM_HARNESS } from '../builtins';
...
/**
 * GLM Harness adapter. Zhipu serves GLM over an Anthropic-compatible
 * Messages endpoint; the native transport reuses the Claude dialect with a
 * different base URL. Owns the BIGMODEL credential slot and the provider
 * model catalog; the generic Rish runtime still owns sessions, workspaces,
 * the agent loop, approvals, the ledger, and proof. Tool semantics
 * (write_file / read_file / list_dir / git_commit) come from the same tool
 * registry as DSH, so approvals and replay protection are shared.
 */
export const GlmHarnessAdapter = {
  manifest: GLM_HARNESS,
  isAvailable: () => LocalRuntime.isAvailable(),
  credentialStatus: () =>
    LocalRuntime.credentialStatusForSlot('BIGMODEL_API_KEY'),
  presentCredentialPrompt: (locale: CredentialPromptLocale) =>
    LocalRuntime.presentCredentialPromptForSlot('BIGMODEL_API_KEY', locale),
  clearCredential: () =>
    LocalRuntime.clearCredentialForSlot('BIGMODEL_API_KEY'),
  complete: (
