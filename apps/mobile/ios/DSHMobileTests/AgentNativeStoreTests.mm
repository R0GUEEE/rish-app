...
- (void)testRoundAndLedgerUseFullLocatorAndCASWithoutPartialPatch {
  NSError *error = nil;
  NSDictionary *transcript = [self.transcripts
      createAgentTranscriptWithRequest:@{
        @"schema_version" : @1,
  ...
  NSDictionary *owner = @{
    @"schema_version" : @1,
    @"task_id" : roundLocator[@"task_id"],
    @"launch_id" : self.wal.launchId,
    @"native_task_id" : @"66666666-6666-4666-8666-666666666666",
    @"owner_generation" : @1,
    @"heartbeat_at" : @"2026-08-30T00:00:00.000Z",
  };
  XCTAssertTrue([self.wal registerNativeTaskId:owner[@"native_task_id"] error:&error]);
  NSDictionary *round = @{
    @"schema_version" : @2,
  ...
  NSDictionary *insertCAS = @{
    ...
    @"expected_transcript_generation" : transcript[@"generation"],
    @"expected_transcript_sha256" : transcript[@"transcript_sha256"],
    @"expected_root_fingerprint_sha256" : [self root][@"root_fingerprint_sha256"],
    @"expected_binding_revision" : @1,
  };
  NSDictionary *inserted = [self.rounds
      createAgentRoundWithInsertCAS:insertCAS exactRoundStart:round error:&error];
  XCTAssertEqualObjects(inserted[@"status"], @"inserted");
  NSDictionary *wrongCAS = @{
    @"schema_version" : @2,
    @"locator" : @{
      @"schema_version" : @2,
      @"task_id" : roundLocator[@"task_id"],
      @"attempt_id" : roundLocator[@"attempt_id"],
    ...
    @"expected_owner_generation" : @1,
    @"expected_launch_id" : owner[@"launch_id"],
    @"expected_native_task_id" : owner[@"native_task_id"],
    @"expected_transcript_generation" : transcript[@"generation"],
    @"expected_transcript_sha256" : transcript[@"transcript_sha256"],
    @"expected_root_fingerprint_sha256" : [self root][@"root_fingerprint_sha256"],
    @"expected_binding_revision" : @1,
  };
  XCTAssertNil([self.rounds casAgentRoundWithCAS:wrongCAS
                                            patch:@{ @"state" : @"cancelled" }
                                            error:&error]);
  XCTAssertEqual(error.code, DSHAgentNativeStoreErrorInvalidArgument);
}

- (void)testWriteReservationIsIdempotentAndEnforcesBatchBounds {
  NSError *error = nil;
  ...
  NSDictionary *reserved = [self.ledger
      reserveWriteBytesForAttemptWithRequest:request error:&error];
  XCTAssertEqualObjects(reserved[@"status"], @"reserved");
  XCTAssertEqualObjects(reserved[@"batch_new"], @32768);
  XCTAssertNil(error);

  NSMutableDictionary *replay = [request mutableCopy];
  replay[@"expected_reserved_write_bytes"] = @32768;
  NSDictionary *replayed = [self.ledger
      reserveWriteBytesForAttemptWithRequest:replay error:&error];
  XCTAssertEqualObjects(replayed[@"batch_new"], @0);
  XCTAssertNil(error);

  NSMutableDictionary *over = [request mutableCopy];
  over[@"expected_reserved_write_bytes"] = @32768;
  over[@"writes"] = @[@{
    @"idempotency_key" :
        @"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd",
    @"relative_path_sha256" :
  ...
  XCTAssertNil([self.ledger reserveWriteBytesForAttemptWithRequest:over
                                                              error:&error]);
  XCTAssertEqual(error.code, DSHAgentNativeStoreErrorCapacity);
}

- (void)testWriteBatchPreservesMixedCallIndicesAndHistoricalReservationVersions {
  __block NSError *error = nil;
  NSMutableDictionary *root = [[self root] mutableCopy];
  root[@"capabilities"] = @[@"file_write"];
  NSString *attemptId = @"22222222-2222-4222-8222-222222222222";
  NSDictionary *transcript = [self.transcripts
      createAgentTranscriptWithRequest:@{
        @"schema_version" : @1,
  ...
  NSDictionary *(^makeIntent)(NSUInteger, NSString *, NSString *) =
      ^NSDictionary *(NSUInteger callIndex, NSString *path, NSString *content) {
    NSString *argumentsJSON = [NSString stringWithFormat:
        @"{\"content\":\"%@\",\"expected_revision\":null,\"path\":\"%@\"}",
        content, path];
    NSString *argumentsSHA = DSHAgentArgumentsSHA256(@"write_file", argumentsJSON,
                                                      &error);
    NSDictionary *baseLocator = @{
      @"schema_version" : @2,
      @"task_id" : @"33333333-3333-4333-8333-333333333333",
      ...
      @"call_id" : [NSString stringWithFormat:@"call-%lu", (unsigned long)callIndex],
      @"idempotency_key" :
    ...
    NSString *pathSHA = DSHAgentHB(@"relative-path", pathBytes, &error);
    NSString *contentSHA = DSHAgentHB(@"file-content", contentBytes, &error);
    NSDictionary *precondition = @{
      @"schema_version" : @2,
      @"kind" : @"write_file",
      @"relative_path_sha256" : pathSHA,
      @"prior" : @{
        @"schema_version" : @1,
        @"kind" : @"absent",
      },
  ...
  NSDictionary *first = makeIntent(1, @"first.txt", @"x");
  NSDictionary *second = makeIntent(3, @"second.txt", @"y");
  // Call index 2 is a non-write provider read. The selected write manifest
  // must preserve the actual write indices (1 and 3) without requiring the
  // read call to be represented as a write entry.
  NSString *readArguments = @"{\"path\":\"middle.txt\"}";
  NSString *readArgumentsSHA = DSHAgentArgumentsSHA256(@"read_file",
                                                        readArguments, &error);
  NSMutableDictionary *readLocator = [@{
    @"schema_version" : @2,
    @"task_id" : first[@"locator"][@"task_id"],
    @"attempt_id" : attemptId,
    @"round_id" : first[@"locator"][@"round_id"],
    @"round_index" : @0,
    @"call_index" : @2,
    @"call_id" : @"call-2",
    @"idempotency_key" :
  ...
  readLocator[@"idempotency_key"] = DSHAgentIdempotencyKeyForLocator(
      readLocator, root[@"root_fingerprint_sha256"], readArgumentsSHA, &error);
  NSDictionary *readIntent = @{
    @"schema_version" : @2,
    @"locator" : readLocator,
    @"row_revision" : @1,
    @"root_fingerprint_sha256" : root[@"root_fingerprint_sha256"],
    ...
    @"transcript_before" : transcript,
    @"name" : @"read_file",
    @"arguments_sha256" : readArgumentsSHA,
    @"precondition" : @{
      @"schema_version" : @1,
      @"kind" : @"read_file",
      @"source_revision" : @"r1",
  ...
  XCTAssertNotNil([self.ledger
      insertAgentExecutionIntentWithInsertCAS:readInsertCAS
                                  argumentsJSON:readArguments
                                   exactIntent:readIntent
                                         error:&error]);
  XCTAssertNil(error);
  NSDictionary *policy = @{
    @"schema_version" : @1,
  ...
  NSDictionary *request = @{
    ...
    @"writes" : @[
      @{
        @"idempotency_key" : first[@"locator"][@"idempotency_key"],
        @"relative_path_sha256" : first[@"precondition"][@"relative_path_sha256"],
        @"content_sha256" : first[@"precondition"][@"content_sha256"],
        @"content_bytes" : @1,
      },
      @{
        @"idempotency_key" : second[@"locator"][@"idempotency_key"],
        @"relative_path_sha256" : second[@"precondition"][@"relative_path_sha256"],
        @"content_sha256" : second[@"precondition"][@"content_sha256"],
        @"content_bytes" : @1,
      },
    ],
    @"intents" : @[first, second],
    @"arguments_json" : @[
      @"{\"content\":\"x\",\"expected_revision\":null,\"path\":\"first.txt\"}",
      @"{\"content\":\"y\",\"expected_revision\":null,\"path\":\"second.txt\"}",
    ],
  };
  NSDictionary *prepared = [self.ledger
      prepareAgentWriteBatchWithRequest:request error:&error];
  XCTAssertEqualObjects(prepared[@"status"], @"prepared");
  XCTAssertNil(error);
  NSDictionary *state = [self.wal snapshotWithError:&error];
  XCTAssertEqual([(NSArray *)state[@"batches"] count], (NSUInteger)1);
  XCTAssertEqual([(NSArray *)state[@"ledger"] count], (NSUInteger)3);
  NSDictionary *batch = state[@"batches"][0];
  XCTAssertEqualObjects(batch[@"reservation_delta_bytes"], @2);
  XCTAssertEqualObjects(batch[@"attempt_reserved_write_bytes"], @2);
  XCTAssertEqualObjects(batch[@"batch_revision"], @1);
  XCTAssertEqualObjects(batch[@"manifest_calls"][0][@"locator"][@"call_index"], @1);
  XCTAssertEqualObjects(batch[@"manifest_calls"][1][@"locator"][@"call_index"], @3);
  NSSet *fileManifestKeys = [NSSet setWithArray:@[
    @"schema_version", @"mutation_kind", @"locator",
    @"precondition_sha256", @"relative_path_sha256", @"prior",
    @"content_sha256", @"content_bytes",
  ]];
  XCTAssertEqualObjects(
      [NSSet setWithArray:[batch[@"manifest_calls"][0] allKeys]],
      fileManifestKeys);
  XCTAssertEqualObjects(
      [NSSet setWithArray:[batch[@"manifest_calls"][1] allKeys]],
      fileManifestKeys);
  XCTAssertEqualObjects([batch[@"manifest_calls"] valueForKey:@"schema_version"],
                        (@[@2, @2]));
  XCTAssertEqualObjects([batch[@"manifest_calls"] valueForKey:@"mutation_kind"],
                        (@[@"file_write", @"file_write"]));
  XCTAssertEqualObjects(batch[@"write_keys"], (@[
    first[@"locator"][@"idempotency_key"],
    second[@"locator"][@"idempotency_key"],
  ]));
  XCTAssertNil(batch[@"reservation_version"]);
  for (NSUInteger index = 0; index < 2; index += 1) {
    NSDictionary *intent = index == 0 ? first : second;
    NSString *expectedPreconditionSHA = DSHAgentHJ(@"tool-precondition", @{
      @"schema_version" : @1,
      @"name" : @"write_file",
      @"precondition" : intent[@"precondition"],
    }, &error);
    XCTAssertEqualObjects(batch[@"manifest_calls"][index]
                               [@"precondition_sha256"],
  ...
  XCTAssertNil(error);
  NSDictionary *genericTerminalCAS = @{
    @"schema_version" : @2,
  ...
  error = nil;
  NSDictionary *genericTerminal = [self.ledger
      casAgentExecutionWithCAS:genericTerminalCAS
                          patch:@{ @"state" : @"cancelled" }
                           error:&error];
  XCTAssertNil(genericTerminal);
  XCTAssertEqual(error.code, DSHAgentNativeStoreErrorConflict);

  // A later batch on the same attempt advances the reservation version and
  // total; the first historical manifest must remain bootstrap-valid.
  NSMutableDictionary *later = [makeIntent(0, @"third.txt", @"z") mutableCopy];
  NSMutableDictionary *laterLocator = [later[@"locator"] mutableCopy];
  laterLocator[@"round_id"] = @"55555555-5555-4555-8555-555555555555";
  NSString *laterArguments = @"{\"content\":\"z\",\"expected_revision\":null,\"path\":\"third.txt\"}";
  NSString *laterArgumentsSHA = DSHAgentArgumentsSHA256(@"write_file", laterArguments,
                                                         &error);
  later[@"arguments_sha256"] = laterArgumentsSHA;
  laterLocator[@"idempotency_key"] = DSHAgentIdempotencyKeyForLocator(
      laterLocator, root[@"root_fingerprint_sha256"], laterArgumentsSHA, &error);
  later[@"locator"] = laterLocator;
  NSString *laterPathSHA = DSHAgentHB(@"relative-path",
                                      [@"third.txt" dataUsingEncoding:NSUTF8StringEncoding],
                                      &error);
  later[@"precondition"] = @{
    @"schema_version" : @2,
    @"kind" : @"write_file",
    @"relative_path_sha256" : laterPathSHA,
    @"prior" : @{@"schema_version" : @1, @"kind" : @"absent"},
    @"content_sha256" : DSHAgentHB(@"file-content",
                                    [@"z" dataUsingEncoding:NSUTF8StringEncoding], &error),
    @"content_bytes" : @1,
  };
  later[@"reserved_write_bytes"] = @1;
  NSDictionary *laterRequest = @{
    @"schema_version" : @1,
    @"task_id" : laterLocator[@"task_id"],
    @"attempt_id" : attemptId,
    @"root_fingerprint_sha256" : root[@"root_fingerprint_sha256"],
    @"binding_revision" : @1,
    @"policy" : policy,
    @"expected_reserved_write_bytes" : @2,
    @"writes" : @[@{
      @"idempotency_key" : laterLocator[@"idempotency_key"],
      @"relative_path_sha256" : later[@"precondition"][@"relative_path_sha256"],
      @"content_sha256" : later[@"precondition"][@"content_sha256"],
      @"content_bytes" : @1,
    }],
    @"intents" : @[later],
    @"arguments_json" : @[laterArguments],
  };
  // The prior generic terminal CAS intentionally failed; do not reuse its
  // NSError slot for this independent successful transaction.
  error = nil;
  NSDictionary *laterPrepared = [self.ledger
      prepareAgentWriteBatchWithRequest:laterRequest error:&error];
  XCTAssertEqualObjects(laterPrepared[@"status"], @"prepared");
  XCTAssertNil(error);
  state = [self.wal snapshotWithError:&error];
  XCTAssertEqual([(NSArray *)state[@"batches"] count], (NSUInteger)2);
  XCTAssertEqualObjects(state[@"batches"][0][@"attempt_reserved_write_bytes"], @2);
  XCTAssertEqualObjects(state[@"batches"][0][@"batch_revision"], @1);
  XCTAssertEqualObjects(state[@"batches"][1][@"attempt_reserved_write_bytes"], @3);
  XCTAssertEqualObjects(state[@"batches"][1][@"batch_revision"], @2);
  XCTAssertEqualObjects(state[@"batches"][1][@"manifest_calls"][0]
                               [@"mutation_kind"],
                        @"file_write");
  XCTAssertNil(state[@"batches"][1][@"reservation_version"]);
  error = nil;
  NSDictionary *cancelled = [self.ledger
      cancelAgentExecutionWithCAS:genericTerminalCAS
                             patch:@{ @"state" : @"cancelled" }
                              error:&error];
  XCTAssertEqualObjects(cancelled[@"row"][@"state"], @"cancelled");
  XCTAssertEqualObjects(cancelled[@"row"][@"receipt"][@"outcome"], @"cancelled");
  XCTAssertNotEqualObjects(cancelled[@"row"][@"transcript_after"], transcript);
  XCTAssertNil(error);
  state = [self.wal snapshotWithError:&error];
  XCTAssertNil(error);
  XCTAssertEqualObjects(state[@"reservations"][0][@"reserved_write_bytes"], @2);
  // Every mutation in the frozen batch is still proven not dispatched, so
  // cancelling one safely closes the entire batch as released.
  XCTAssertEqualObjects(state[@"batches"][0][@"effect_gate"], @"released");
  XCTAssertNil(error);
}
