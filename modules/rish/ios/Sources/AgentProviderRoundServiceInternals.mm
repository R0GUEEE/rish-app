...
static NSString *DSHProviderToolDescription(NSString *name, NSString *fallback) {
  ...
  dispatch_once(&onceToken, ^{
    descriptions = @{
      ...
      @"start_guest_cgi" : @"Start the local HTTP preview service using an HTML file and a BusyBox /bin/sh backend script. This tool runs the service; do not ask the user to run node server.js or double-click files on iOS. Node.js, npm servers, and arbitrary long-running processes are not supported by this tool. GET / serves the HTML; POST /api executes backend.sh with the request-body file as $1 and the mutable initial-data file as $2, and returns stdout as JSON. For a static preview, create a minimal backend.sh that prints {}. HTML must be self-contained (inline CSS/JS, at most 32 KiB); backend.sh is at most 8 KiB. Return the preview URL only after this tool succeeds. The service is local to this device, is temporary, and may stop when the app is backgrounded. All paths are relative to the workspace root exactly as used with read_file (e.g. \"index.html\", \"backend.sh\"); never prefix them with a directory such as /workspace. Each sha256 must be the value read_file returned for that file. When there is no seed data, pass null for both initial_data_path and initial_data_sha256.",
